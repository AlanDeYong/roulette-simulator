 ...
 * - Let `peakBankroll` track the highest bankroll achieved to determine resets.
 * - On a Win (Jackpot or Line):
 *   - Reset `units` back to 1 and `isPressed` to false ONLY when the session's peak profit is reached (`bankroll > peakBankroll`).
 *   - ELSE, rebet and double up all units (`units *= 2`, set `isPressed = true`).
 * - On a Loss:
 *   - Increase all bets by their respective base bet amount (`units += 1`). Set `isPressed = false`.
 *
 * The Goal:
 * - To capitalize on consecutive wins by pressing (doubling) bets, while steadily recovering from losses by adding units linearly. A reset only occurs when a new session peak bankroll is achieved.
 */
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    if (spinHistory.length > 0) {
        ...
        if (bankroll > state.peakBankroll) {
            // Reset only when session's peak profit is reached
            state.peakBankroll = bankroll;
            state.units = 1;
            state.isPressed = false;
            ...
            if (hitType === 'jackpot' || hitType === 'line') {
                // Rebet and double up all units
                state.units *= 2;
                state.isPressed = true;
                state.lastSpinWasWin = true;
            } else if (hitType === 'loss') {
                // Increase all bets by their respective base bet amount
                state.units += 1;
                state.isPressed = false;
                state.lastSpinWasWin = false;
            }
