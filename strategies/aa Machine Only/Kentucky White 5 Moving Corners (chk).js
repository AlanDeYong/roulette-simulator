/**
 * Strategy: Moving Corners (Modified - Max 5 Corners)
 ...
 * 5. Stage 5: 5 Corners @ 8x Base Unit (Win -> Reset / Loss -> Go to Stage 6)
 * 6. Stage 6: 5 Corners @ 16x Base Unit (Double bet size on same coverage)
 * - Win at Stage 6: Hard Reset to Stage 1 to lock profit/minimize risk.
 ...
 * but expand coverage (up to ~54%) and bet size to recover if early bets fail.
 */
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    if (spinHistory.length > 0) {
        ...
        if (won) {
            ...
            else if (state.stage === 4) state.stage = 5;  // 5 -> 5 corners (Double Bet)
            else if (state.stage === 5) state.stage = 6;  // 5 -> 5 corners (Double again)
    ...
    switch (state.stage) {
        ...
        case 5: cornersCount = 5; multiplier = 8; break; // Cap at 5 corners, double bet
        case 6: cornersCount = 5; multiplier = 16; break; // Cap at 5 corners, double again
    }
