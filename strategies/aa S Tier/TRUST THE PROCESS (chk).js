 ...
 * - Note: Progression heavily modified via custom ruleset.
 ...
 * - Subsequent Losses (Stage 3+): Rebet all 3 Zones and double the unit multiplier indefinitely.
 * - On Win (Net Profit > 0): 
 ...
 * * * THE GOAL:
 * - Recover drawdowns aggressively by systematically expanding board coverage and multiplying bet sizes, immediately resetting to a safe baseline once a new session high is established.
 */
    ...
    if (spinHistory.length > 0) {
        ...
        if (isWin) {
            ...
            if (state.stage === 1) {
                ...
                state.multiplier *= 2; // Add Z3 (if stage 2) and double up bets indefinitely
            }
        }
    }
