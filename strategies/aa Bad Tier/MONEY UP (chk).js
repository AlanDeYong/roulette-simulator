/**
 * Roulette Strategy: Six-Line Pattern Progression
 ...
 * - Starting numbers for Forward direction: 1, 7, 13, 19, 25.
 * - Starting numbers for Reverse direction: 31, 25, 19, 13, 7.
 * - Outcomes defined:
 ...
 * - Triggers:
 * - On Win: If a new session peak profit is reached, reset progression to Step 0 and switch betting direction (opposite end of the felt). If not at peak profit, and the previous level was reached via double-up (Steps 2, 4, 5, 6), go down 1 level. Otherwise, rebet.
 ...
 * * The Goal:
 * To hit covered numbers enough times to achieve a session peak profit, at which point the strategy resets, captures profit, and attacks from the opposite side of the table.
 */
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    const forwardStarts = [1, 7, 13, 19, 25];
    const reverseStarts = [31, 25, 19, 13, 7];
    const startNumbers = state.isReverse ? reverseStarts : forwardStarts;
    ...
    for (let i = 0; i < currentProg.numPatterns; i++) {
        const S = startNumbers[i]; // Top-left number for this 6-number block
