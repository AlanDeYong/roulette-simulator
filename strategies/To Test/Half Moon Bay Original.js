 ...
 * The Logic:
 * 1. Sets:
 * - Half Moon (H): [3, 5, 6, 7, 8, 11, 13, 15, 17, 20, 21, 22, 24, 26, 28, 30, 33, 34]
 * - Opposite (O): All other numbers 1-36.
 * 2. Trigger:
 * - Wait for 3 consecutive non-zero numbers to hit one set (e.g., 3 hits on H).
 * - Bet on the Opposite set + Zero (19 numbers total).
 *
 ...
 *
 * The Goal: 
 * - Grind consistent small profits (covering 19/37 numbers).
...
function bet(spinHistory, bankroll, config, state) {
    ...
    // --- Constants ---
    const HALF_MOON_SET = [3, 5, 6, 7, 8, 11, 13, 15, 17, 20, 21, 22, 24, 26, 28, 30, 33, 34];
    ...
    if (state.phase === 'BETTING') {
        ...
        // Generate Bet Array
        // We bet Straight Up ('number') on all 19 target numbers
