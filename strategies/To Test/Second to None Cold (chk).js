/**
 * Strategy: Second to None (Dynamic 37-Spin History)
 * * Source: The Roulette Master (YouTube) - https://www.youtube.com/watch?v=QtHZfh22aUc
 * * The Logic: 
 ...
 * the "coldest" (lowest frequency) fully contained corner, and the two coldest remaining 
 * single numbers in that dozen. A "reset" is triggered when a single number hits (resulting 
 * in a bankroll increase), forcing the system to recalculate the coldest numbers based on 
 ...
 * * The Goal: 
 * Exploit "sleeping" sections of the board while using corners as break-even safety nets.
...
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    // In this system, a corner hit breaks even, while a single number hit profits.
    ...
    // 4. Calculate Dynamic Layout if null
    if (!state.layout) {
        ...
        // Helper function to find the coldest layout in a specific dozen
        const getColdestDozenLayout = (dozenStart, validTopLeftCorners) => {
            let bestCorner = validTopLeftCorners[0];
            let lowestCornerFreq = Infinity;

            // Find coldest corner
            for (let c of validTopLeftCorners) {
                let cornerFreq = freqs[c] + freqs[c+1] + freqs[c+3] + freqs[c+4];
                if (cornerFreq < lowestCornerFreq) {
                    lowestCornerFreq = cornerFreq;
                    bestCorner = c;
                }
            }
            ...
            // Find 2 coldest single numbers not inside the chosen corner
            ...
            // Sort available numbers by frequency (ascending)
            availableNumbers.sort((a, b) => freqs[a] - freqs[b]);
        ...
        state.layout = [
            ...getColdestDozenLayout(1, d1Corners),
            ...getColdestDozenLayout(13, d2Corners),
            ...getColdestDozenLayout(25, d3Corners)
        ];
    }
