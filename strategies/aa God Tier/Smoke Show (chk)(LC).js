/**
 * ROULETTE STRATEGY: Smoke Show Strategy (V4 - Alternating Columns)
 * 
 * Source:
 ...
 * - The strategy places 6 non-overlapping Corner bets covering 24 numbers total (alternating blocks).
 * - Corner anchors (top-left numbers): [1, 8, 13, 20, 25, 32]
 *   - Corner 1:  [1, 2, 4, 5]
 *   - Corner 8:  [8, 9, 11, 12]
...
function bet(spinHistory, bankroll, config, state, utils) {
  ...
  const initialCorners = [1, 8, 13, 20, 25, 32];

  // FORCE STATE RESET: Bumping version to 4 so the simulator drops the old array and uses the new one.
