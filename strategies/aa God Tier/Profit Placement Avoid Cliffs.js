/**
 * STRATEGY: DYNAMIC HOT SPLITS + ZERO + JACKPOT NUMBERS + VIRTUAL SAFETY NET
 * * Source: Custom / User Provided
 * * The Logic: 
 * - Tracking Phase: Observes the first 20 spins without betting.
 * - Selection: Selects non-overlapping splits with the highest hit frequency. 
 * - Jackpot Feature: When a split wins, the non-hitting number becomes a straight-up "Jackpot" bet (Max 3). 
 * The active splits are reduced to maintain exactly 11 active bet units (Splits + Jackpots + Zero).
 * Jackpot numbers persist through losses until a session reset.
 * * The Progression & Virtual Mode:
 * - Levels 1-6: Increase base bet by 1 unit after a loss (1u, 2u, 3u, 4u, 5u, 6u).
 * - Levels 7-8: Accelerate bet size (12u, 24u).
 * - VIRTUAL MODE: If Level 5 loses, actual betting stops. The script enters "Virtual Mode". 
 * It tracks theoretical bets in the background without placing real money until 2 CONSECUTIVE WINS manifest.
 * Once 2 consecutive wins occur, real betting resumes and the progression resets to Level 1.
 * - Rebet Phase: ANY win triggers a Rebet phase at the same level (using the same splits/jackpots).
 * * The Goal:
 * - Target Profit: Dynamic Trailing Peak. 
 * - Resets to base (Level 1) and clears Jackpots ONLY when recovering from a drawdown 
 * and a win brings the bankroll to within 2% of the highest recorded session peak.
 */

function bet(spinHistory, bankroll, config, state, utils) {
    try {
        // --- 1. Initialize State ---
        if (!state.initialized) {
            state.level = 1;
            state.rebetPhase = false;
            state.virtualMode = false;   // NEW: Toggles virtual betting
            state.virtualWins = 0;       // NEW: Tracks consecutive virtual wins
            state.jackpotNumbers = []; 
            state.excludeSplit = null; 
            state.previousSplits = []; 
            state.initialBankroll = bankroll; 
            state.peakBankroll = bankroll; 
            state.initialized = true;
        }

        // --- 2. Tracking Phase (First 20 Spins) ---
        if (spinHistory.length < 20) {
            return [];
        }

        // --- 3. Process Previous Result ---
        if (spinHistory.length > 20) {
            const lastResult = spinHistory[spinHistory.length - 1];
            const lastWinNum = lastResult.winningNumber;
            let wonLastSpin = false;
            let winningSplitValue = null;

            if (state.lastBets) {
                for (const b of state.lastBets) {
                    if (b.type === 'number' && b.value === lastWinNum) {
                        wonLastSpin = true;
                    }
                    if (b.type === 'split' && b.value.includes(lastWinNum)) {
                        wonLastSpin = true;
                        winningSplitValue = b.value;
                    }
                }
            }

            // --- DYNAMIC TRAILING PEAK LOGIC ---
            if (state.peakBankroll === undefined) state.peakBankroll = state.initialBankroll;
            if (state.initialBankroll === undefined) state.initialBankroll = bankroll;
            
            // Only update peak if we are NOT in virtual mode
            if (!state.virtualMode && bankroll > state.peakBankroll) {
                state.peakBankroll = bankroll;
            }

            const margin = state.initialBankroll * 0.02;
            const resetTarget = state.peakBankroll - margin;

            let justFinishedRebet = false;

            if (state.rebetPhase) {
                state.rebetPhase = false;
                justFinishedRebet = true; 
            } 

            // --- STATE TRANSITIONS (Forked for Virtual vs Real) ---
            if (state.virtualMode) {
                // VIRTUAL MODE Logic
                if (wonLastSpin) {
                    state.virtualWins++;
                    state.rebetPhase = true;
                    state.excludeSplit = winningSplitValue; 
                    
                    // Trigger Jackpot Number generation if a Split won
                    if (winningSplitValue) {
                        const nonHitNum = (winningSplitValue[0] === lastWinNum) ? winningSplitValue[1] : winningSplitValue[0];
                        if (!state.jackpotNumbers.includes(nonHitNum) && state.jackpotNumbers.length < 3) {
                            state.jackpotNumbers.push(nonHitNum);
                        }
                    }

                    // Check Exit Condition
                    if (state.virtualWins >= 2) {
                        state.virtualMode = false; // Exit virtual mode
                        state.virtualWins = 0;     // Reset counter
                        state.level = 1;           // Reset bet amount to base level
                    }
                } else {
                    // Virtual Loss
                    state.virtualWins = 0;
                    state.rebetPhase = false;
                    state.excludeSplit = null;
                }
            } else {
                // REAL MODE Logic
                // --- GOAL OVERRIDE (Recovery Reset) ---
                // Only resets if we are deep in a progression (Level > 1) and have recovered
                if (state.level > 1 && bankroll >= resetTarget && (wonLastSpin || justFinishedRebet)) {
                    // Target met! Reset everything to base.
                    state.level = 1;
                    state.rebetPhase = false;
                    state.excludeSplit = null;
                    state.previousSplits = [];
                    state.jackpotNumbers = []; // Clear jackpots on successful recovery
                } else {
                    // Target not met, or we are actively winning at Level 1.
                    if (wonLastSpin) {
                        // ANY win triggers/maintains Rebet Phase
                        state.rebetPhase = true;
                        state.excludeSplit = winningSplitValue; 
                        
                        // Trigger Jackpot Number generation if a Split won
                        if (winningSplitValue) {
                            const nonHitNum = (winningSplitValue[0] === lastWinNum) ? winningSplitValue[1] : winningSplitValue[0];
                            if (!state.jackpotNumbers.includes(nonHitNum) && state.jackpotNumbers.length < 3) {
                                state.jackpotNumbers.push(nonHitNum);
                            }
                        }
                    } else {
                        // Loss: Increase Level
                        state.level++;
                        if (state.level > 8) state.level = 1; 
                        state.rebetPhase = false;
                        state.excludeSplit = null;
                        
                        // NEW: Enter virtual mode if we just lost at Level 5
                        if (state.level === 6) {
                            state.virtualMode = true;
                            state.virtualWins = 0;
                        }
                    }
                }
            }
        }

        // --- 4. Progression Calculation ---
        const minInside = config.betLimits.min;
        let multiplier = state.level;
        if (state.level === 7) multiplier = 12;
        if (state.level === 8) multiplier = 24;

        let chipAmount = minInside * multiplier;
        chipAmount = Math.max(chipAmount, config.betLimits.min);
        chipAmount = Math.min(chipAmount, config.betLimits.max);

        // --- 5. Determine Active Splits ---
        let activeSplits = [];
        
        // Dynamic target: Subtract jackpots from the 10 splits to maintain 11 total bets
        const TARGET_SPLITS = 10 - state.jackpotNumbers.length;

        if (state.rebetPhase) {
            if (!state.previousSplits) state.previousSplits = [];
            
            // Retain previous splits, filtering out the one that just won
            activeSplits = state.previousSplits.filter(s => {
                if (state.excludeSplit && s[0] === state.excludeSplit[0] && s[1] === state.excludeSplit[1]) {
                    return false;
                }
                return true;
            });
            
            // If we need to pull new splits to hit the dynamic TARGET_SPLITS count
            if (activeSplits.length < TARGET_SPLITS) {
                 const window = spinHistory.slice(-20);
                 const counts = {};
                 for (let i = 0; i <= 36; i++) counts[i] = 0;
                 window.forEach(spin => { if (spin.winningNumber !== undefined) counts[spin.winningNumber]++; });
                 
                 const allSplits = getAllSplits();
                 const currentSet = new Set(activeSplits.map(s => s.toString()));
                 if (state.excludeSplit) currentSet.add(state.excludeSplit.toString());

                 const coveredNumbers = new Set();
                 activeSplits.forEach(s => {
                     coveredNumbers.add(s[0]);
                     coveredNumbers.add(s[1]);
                 });
                 // Prevent overlap with Jackpots
                 state.jackpotNumbers.forEach(j => coveredNumbers.add(j));

                 // Loop to fill exactly up to TARGET_SPLITS
                 while (activeSplits.length < TARGET_SPLITS) {
                     let bestNewSplit = null;
                     let bestScore = -Infinity;

                     for (const split of allSplits) {
                         if (currentSet.has(split.toString())) continue;
                         if (coveredNumbers.has(split[0]) || coveredNumbers.has(split[1])) continue;
                         
                         const n1 = split[0];
                         const n2 = split[1];
                         const totalFreq = counts[n1] + counts[n2];
                         let score = totalFreq * 10;
                         if (counts[n1] === 0 || counts[n2] === 0) score = -100 + totalFreq;
                         
                         if (score > bestScore) {
                             bestScore = score;
                             bestNewSplit = split;
                         }
                     }
                     
                     if (bestNewSplit) {
                         activeSplits.push(bestNewSplit);
                         currentSet.add(bestNewSplit.toString());
                         coveredNumbers.add(bestNewSplit[0]);
                         coveredNumbers.add(bestNewSplit[1]);
                     } else {
                         break; // Failsafe
                     }
                 }
            }
            
            // Failsafe clamp if activeSplits somehow exceeds target
            if (activeSplits.length > TARGET_SPLITS) {
                activeSplits = activeSplits.slice(0, TARGET_SPLITS);
            }
            
        } else {
            // --- Standard Calculation (After a Loss) ---
            const window = spinHistory.slice(-20);
            const counts = {};
            for (let i = 0; i <= 36; i++) counts[i] = 0;
            
            window.forEach(spin => {
                if (spin.winningNumber !== undefined) counts[spin.winningNumber]++;
            });

            const allSplits = getAllSplits();
            let candidates = [];

            for (const split of allSplits) {
                const n1 = split[0];
                const n2 = split[1];
                const f1 = counts[n1];
                const f2 = counts[n2];
                const totalFreq = f1 + f2;

                let qualityScore = totalFreq * 10; 
                if (f1 === 0 || f2 === 0) {
                    qualityScore = -100 + totalFreq; 
                }

                candidates.push({ value: split, score: qualityScore });
            }

            candidates.sort((a, b) => b.score - a.score);

            const coveredNumbers = new Set();
            // Critical: Block persisting Jackpots from being covered by new splits
            state.jackpotNumbers.forEach(j => coveredNumbers.add(j));
            
            activeSplits = [];

            for (const cand of candidates) {
                if (activeSplits.length >= TARGET_SPLITS) break;

                const s = cand.value;
                if (!coveredNumbers.has(s[0]) && !coveredNumbers.has(s[1])) {
                    activeSplits.push(s);
                    coveredNumbers.add(s[0]);
                    coveredNumbers.add(s[1]);
                }
            }
        }
        
        state.previousSplits = activeSplits;

        // --- 6. Construct Final Bets ---
        const bets = [];

        // Add Zero
        if (bankroll >= chipAmount) {
            bets.push({ type: 'number', value: 0, amount: chipAmount });
        }

        // Add Splits
        for (const val of activeSplits) {
            if (bankroll >= chipAmount) {
                bets.push({ type: 'split', value: val, amount: chipAmount });
            }
        }

        // Add Jackpot Numbers
        for (const jn of state.jackpotNumbers) {
             if (bankroll >= chipAmount) {
                 bets.push({ type: 'number', value: jn, amount: chipAmount });
             }
        }

        // CRITICAL: Always save the calculated bets to state so the algorithm 
        // can evaluate if they "won" on the next spin.
        state.lastBets = bets;

        // NEW: If we are in virtual mode, return an empty array to the simulator 
        // so NO real money is placed.
        if (state.virtualMode) {
            return [];
        }

        return bets;
    } catch (e) {
        if (utils && utils.log) utils.log("ERROR IN STRATEGY: " + e.message);
        console.error("CRITICAL STRATEGY ERROR:", e);
        return [];
    }
}

// --- Helper ---
function getAllSplits() {
    const splits = [];
    for (let row = 1; row <= 34; row += 3) {
        splits.push([row, row + 1]);     
        splits.push([row + 1, row + 2]); 
    }
    for (let n = 1; n <= 33; n++) {
        splits.push([n, n + 3]);
    }
    return splits;
}