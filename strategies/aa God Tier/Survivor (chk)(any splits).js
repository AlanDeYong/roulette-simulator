/**
 * SURVIVOR ROULETTE STRATEGY (Corrected: Randomized & Non-Overlapping Splits)
 * 
 * Source: https://www.youtube.com/watch?v=ow5RwEMeTxY (The Roulette Master / John Lewis)
 * 
 * The Full Logic in details:
 * - Placements: 1 unit on the Zero (0), and 3 random, strictly non-overlapping splits inside each of the 3 dozens. Total of 10 bets.
 * - Base Level Win: If a split hits while at the base bet level, it is removed and a new random, non-overlapping split is generated in the same dozen.
...
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    const getAllSplitsForDozen = (dozen) => {
        ...
        for (let i = start; i <= end; i++) {
            // Horizontal split (excluding the end of a row)
            if (i % 3 !== 0) splits.push([i, i + 1]);
            // Vertical split (excluding the bottom row of the dozen)
    ...
    // Picks a random split that does not overlap with any currently covered numbers
    const getRandomNonOverlappingSplit = (activeBets, dozen) => {
    ...
    // Helper to initialize the baseline Survivor state with random non-overlapping splits
    const initializeState = () => {
        ...
        for (let dozen = 1; dozen <= 3; dozen++) {
            for (let i = 0; i < 3; i++) {
                const newSplit = getRandomNonOverlappingSplit(state.activeBets, dozen);
                if (newSplit) {
    ...
    if (typeof state.referenceBankroll === 'undefined') {
        ...
        // "Every time you get new session profit you're just going to reset"
        state.referenceBankroll = bankroll;
        initializeState();
    }
            ...
            for (let b of state.activeBets) {
                if (isWin(b, lastNum)) {
                    if (state.hasLost) {
                        ...
                        if (b.type === 'split') {
                            // Find a new random non-overlapping split
                            // We pass all other bets *except* this one to ensure we don't overlap with them
                            const otherBets = state.activeBets.filter(bet => bet.id !== b.id);
                            const newSplit = getRandomNonOverlappingSplit(otherBets, b.dozen);
