/**
 * 6 Corners to Straight Roulette Strategy
 ...
 * - On a win (first time or reset): Keep the same bets (rebet), do not convert.
 * - On hitting a corner AFTER a loss (recovery mode): Convert that specific winning 
 * corner into 3 straight bets. The straight bet amount is the corner bet amount / 4. 
 * Maximum 3 corners converted.
 * * The Goal:
...
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    if (!state.initialized) {
        ...
        state.addedUnits = 0; // Tracks the progression increases
        state.activeCorners = []; 
    ...
    if (state.activeCorners.length > 0) {
        ...
        if (netProfit < 0) {
            ...
            state.addedUnits += 1; // This acts as a multiplier later (adds 4 to corners)
        } else if (netProfit > 0) {
            // Win
            if (state.mode === 'recovery') {
                if (bankroll >= state.targetBankroll) {
                    ...
                    // Won, but still in recovery. Check for corner conversion.
    ...
    // 5. Generate Bets with Unit Logic: 4 -> 8 -> 12...
