 ...
 * - 1 unit on Line (Double Street) 13-18
 * - 1 unit on Street 13-15
 * - 1 unit on Street 16-18
 * * Conditions & Triggers:
 ...
 * - After a Partial Loss: Saves current state to history, then increases bets by one unit level (respects incrementMode).
...
function bet(spinHistory, bankroll, config, state, utils) {
    ...
    const baseBets = [
        ...
        { type: 'line', value: 13, units: 1 },
        { type: 'street', value: 13, units: 1 },
        { type: 'street', value: 16, units: 1 },
    ...
    if (spinHistory.length > 0) {
        ...
        if (resultType === 'win') {
            ...
            // Increase by one unit level
            state.currentBets = state.currentBets.map((bet, i) => {
                const mode = config.incrementMode || 'base';
                const minInc = config.minIncrementalBet !== undefined ? config.minIncrementalBet : 1;
                
                const increment = mode === 'base' ? (baseBets[i].units * unit) : minInc;
                return { ...bet, amount: bet.amount + increment };
            });
